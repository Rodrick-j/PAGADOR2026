...
export class ViaticoGeneralReport {
  static async creaPDF(data: any, template: string, res: Response): Promise<any> {//(data: ViaticoData, template: string, res: Response)
    try {
      ...
      const footerTemplate = `<table width="100%" cellspacing="0" cellpadding="0" style="padding: 0 20px 0 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', 'Oxygen', 'Ubuntu', 'Cantarell', 'Fira Sans', 'Droid Sans', 'Helvetica Neue', sans-serif; color:#8a8a8a;"><tbody>
        <tr>
        <td width="33.33%" style="padding:4px;font-size:8px;border-top:solid 0px lightgrey;text-align:left;">Generado por:&nbsp;&nbsp;<span class="font-thin">${data.info.nombre}</span></td>
     ...
     /* const browser = await puppeteer.launch({
      ...
      const pdfBuffer = await page.pdf({
        ...
        margin: { top: '0.5cm', right: '0.5cm', bottom: '0.9cm', left: '0.5cm' },
        scale: 1,
